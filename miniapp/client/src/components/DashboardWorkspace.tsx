...
export function DashboardWorkspace({ profile, countries, records, onNavigate }: { profile: Profile; countries: Country[]; records: RecordResponse | null; onNavigate: (action: DashboardAction) => void }) {
  ...
  return <section className="panel-enter max-w-5xl">
    <section className={`vip-dashboard-stage ${flipped ? "is-flipped" : ""} relative`} aria-label="VIP account card">
      ...
      >
          ...
          <div className="vip-dashboard-card vip-glass-card relative overflow-hidden rounded-[30px] p-5 text-white sm:p-7">
            ...
            <div className="relative z-10">
              ...
              <div className="mt-5 text-center">
                ...
                <h1 className="mt-1.5 truncate text-[25px] font-extrabold tracking-[-.035em] sm:text-[30px]">{profile.user.first_name || "TgFox user"}</h1>
                {profile.user.username && <p className="mt-1 truncate text-[11px] font-medium text-cyan-50/55">@{profile.user.username}</p>}
              </div>

              <div className="vip-balance-panel mt-5 rounded-[22px] border border-white/[.17] bg-black/[.14] p-4 backdrop-blur-2xl">
                <div className="flex items-end justify-between gap-3">
                  <div>
                    <p className="text-[9px] font-bold tracking-[.17em] text-white/50">AVAILABLE BALANCE</p>
                    <p className="mt-1 text-[30px] font-black tracking-[-.045em] tabular-nums sm:text-[34px]">{cash(displayBalance)}</p>
                  </div>
                  <div className="mb-1 grid h-10 w-10 place-items-center rounded-xl border border-cyan-100/[.18] bg-cyan-100/[.08] text-cyan-100"><CircleDollarSign size={19} /></div>
                </div>
              </div>

              <div className="vip-progress-panel mt-3 rounded-[22px] border border-white/[.14] bg-white/[.06] p-4 backdrop-blur-2xl">
                ...
                <div className="vip-progress-track mt-3 h-1.5 overflow-hidden rounded-full bg-white/[.10]">
        ...
        <div className="vip-card-face vip-card-back">
          <div className="vip-dashboard-card vip-glass-card relative flex min-h-[365px] flex-col justify-between overflow-hidden rounded-[30px] p-5 text-white sm:p-7">
            ...
            <div className="relative z-10 flex flex-col items-center py-3">
              <div className="vip-avatar-wrap relative grid h-28 w-28 place-items-center rounded-full p-[3px]">
              ...
              <h2 className="mt-4 max-w-[90%] truncate text-xl font-extrabold">{profile.user.first_name || "TgFox user"}</h2>
              {profile.user.username && <p className="mt-1 text-xs text-white/50">@{profile.user.username}</p>}
              <p className="mt-4 text-[9px] font-bold tracking-[.16em] text-white/40">YOUR PREMIUM ACCESS</p>
            </div>
    ...
    <style>{`
      .vip-dashboard-stage { perspective: 1500px; min-height: 365px; }
      .vip-card-inner { min-height: 365px; transform-style: preserve-3d; transition: transform 900ms cubic-bezier(.2,.75,.2,1); }
      .vip-dashboard-stage.is-flipped .vip-card-inner { transform: rotateY(180deg); }
      .vip-card-face { position: absolute; inset: 0; width: 100%; min-height: 365px; backface-visibility: hidden; -webkit-backface-visibility: hidden; }
      .vip-card-back { transform: rotateY(180deg); }
      .vip-card-front > .vip-dashboard-card, .vip-card-back > .vip-dashboard-card { min-height: 365px; }
      .vip-glass-card { background: linear-gradient(135deg, rgba(31,93,176,.78), rgba(5,24,58,.82) 72%); border: 1px solid rgba(255,255,255,.18); box-shadow: inset 0 1px 0 rgba(255,255,255,.10), 0 26px 70px rgba(4,25,66,.26), 0 0 0 1px rgba(103,190,255,.05); backdrop-filter: blur(18px); -webkit-backdrop-filter: blur(18px); }
