...
async def api_countries(user: dict = Depends(require_webapp_user)) -> JSONResponse:
    """Return the Mini App country list using the same canonical stock logic
    as GET /api/v1/countries.

    This intentionally reuses country_service.fetch_all_countries() instead
    of maintaining a second MongoDB stock filter here. This keeps the Bot,
    Public API and Mini App on the same sellable-stock source.
    """
    from server.utils.database.countrydb import get_all_countries
    from server.services.country_service import fetch_all_countries

    uid = int(user["id"])
    countries = await get_all_countries(buy_only=True)

    # Canonical stock source: exactly the same service used by
    # GET /api/v1/countries. Do not duplicate stock filters here.
    available = await fetch_all_countries()
    stock_map = {
        str(c.get("code") or "").upper(): int(c.get("stock") or 0)
        for c in available
        if c.get("code")
    }

    # Optional per-user discount preview.
    try:
    ...
    for c in countries or []:
        ...
        # Match the canonical /api/v1/countries availability behavior.
        if code not in stock_map or stock_map[code] <= 0:
            continue
