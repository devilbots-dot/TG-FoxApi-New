...
async def api_countries(user: dict = Depends(require_webapp_user)) -> JSONResponse:
    """List active countries using the MongoDB inventory as the source of truth.

    The Telegram bot and public API can update their own in-process memstore when
    stock is uploaded. The Mini App may be served by a different worker/process,
    so relying only on RAM here can leave its stock count stale until restart.
    This endpoint therefore reads the current sellable inventory directly from
    ``session_accounts`` on every market refresh.
    """
    ...
    # IMPORTANT: Mini App stock must see accounts uploaded by the admin pipeline
    # even when that upload happened in another process/worker.  MongoDB is the
    # shared source of truth; RAM memstore is only a performance cache.
    stock_map: dict[str, int] = {}
    pipeline = [
        {
            "$match": {
                "sold": False,
                "inventory_state": {"$ne": "bin"},
                "spam_status": {"$in": ["clean", "temporary_spam", "permanent_spam"]},
            }
        },
        {"$group": {"_id": "$country_code", "count": {"$sum": 1}}},
    ]
    async for row in collection("session_accounts").aggregate(pipeline):
        code = str(row.get("_id") or "").upper()
        if code:
            stock_map[code] = int(row.get("count") or 0)
    ...
    for c in countries or []:
        ...
        if apply_discount_context and discount_context:
            try:
                ...
                final = base
        real_stock = int(stock_map.get(code, 0))
        # Keep the Mini App's displayed number identical to the Bot/Public API
        # when a country is configured for fake display stock. Purchases still
        # use real session_accounts inventory on the server.
        display_stock = displayed_stock(c, real_stock)
        items.append({
            "code": code,
    ...
    response = JSONResponse({"countries": items})
